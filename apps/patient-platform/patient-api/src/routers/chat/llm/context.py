...
class ContextLoader:
    ...
    Documents are cached at the class level for performance.
    """
    
    # Class-level cache for documents - loaded once and reused
    ...
    def __init__(self, directory: str):
        ...
        # Load documents into cache if not already loaded
    ...
    def _load_base_documents_into_cache(self) -> str:
        """Loads all base documents once and caches them. This is only called on first initialization."""
        print("[CTX] Loading base documents into cache...")
    ...
    def _get_cached_base_documents(self) -> str:
        """Returns the cached base documents without reloading."""
    ...
    def load_context(self, symptoms: List[str] = None) -> str:
        ...
        Base documents are served from cache for performance.
        """
        start_time = time.time()
        print(f"[CTX] Building complete system prompt for symptoms: {symptoms}")
        
        # Step 1: Get cached base documents (no reloading)
        base_prompt = self._get_cached_base_documents()
        
        # Step 2: Append Pinecone RAG results (with Redis caching)
        complete_prompt = self._append_rag_results(base_prompt, symptoms)
        
        total_time = time.time() - start_time
        print(f"[CTX] Context built in {total_time:.3f}s (cached: {ContextLoader._base_documents_cache is not None})")
        
        return complete_prompt

    @classmethod
    def clear_cache(cls):
        """Clears the cached documents. Useful if documents are updated."""
