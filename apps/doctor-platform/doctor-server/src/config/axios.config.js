...
const rawApiBase = process.env.DOCTOR_API_BASE;
const sanitizedApiBase = (typeof rawApiBase === 'string' ? rawApiBase.trim() : '') || 'http://localhost:8001';
...
try {
  ...
  console.error(`Invalid DOCTOR_API_BASE provided: ${JSON.stringify(rawApiBase)} → using sanitized: ${JSON.stringify(sanitizedApiBase)}`);
}
