...
const AddStaffModal: React.FC<AddStaffModalProps> = ({ open, onClose }) => {
  const [formData, setFormData] = useState<AddStaffRequest>({
    firstName: '',
    lastName: '',
    email: '',
    role: '',
  ...
  const [errors, setErrors] = useState<Partial<AddStaffRequest>>({});
  
  const addStaffMutation = useAddStaff();
  
  const handleInputChange = (field: keyof AddStaffRequest, value: string) => {
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<AddStaffRequest> = {};
